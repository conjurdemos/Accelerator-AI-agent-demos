"""
LangGraph agent that uses two tools:
...
async def main() -> None:
  psql_uri = f"postgresql+psycopg2://{USER}:{PASSWORD}@{HOST}:{PORT}/{DATABASE}"
  logging.debug(f"PSQL URI: {psql_uri}")
  ...
  system_message = f"""You are a database expert agent designed to interact with a SQL database.
Given an input question, create a syntactically correct SQL query to run, then look at the results
...
query to at most 100 results using the LIMIT clause as per SQL.
...
the question. Wrap each column name in double quotes (") to denote them as delimited identifiers.
...
"""
  ...
  def sql_tool(user_input):
    ...
    """
    try:
        # Invoke the sql_agent with the user input (SQL query)
  ...
  tools = [tavily_tool_instance, sql_tool]
